...
#pragma endregion ListBox
	...
	case WM_COMMAND:
		...
		if ((HWND)lParam == hWndEdit1) SetFocus(hWndComboBoxDisk1);
			...
			{
			case IDM_ABOUT:
				MessageBox(hWnd, L"Program: Total V\nDeveloper: Nine Variant\nE-MAIL: 404.variant@gmail.com", L"About", MB_OK);
				break;
			...
			case ID_COMBO_BOX_1:
				...
				{
				/*	TCHAR Buffer[256], Msg[512];
								int nIndex = (int)SendMessage(hWndComboBoxDisk1, (UINT)CB_GETCURSEL, (WPARAM)0, (LPARAM)0);

					SendMessage((HWND)lParam, (UINT)CB_GETLBTEXT, (WPARAM)nIndex, (LPARAM)Buffer);

					wsprintf(Msg, TEXT("Выбрана строка: %s"), Buffer);
					MessageBox(hWnd, Msg, GAppname, MB_OK | MB_ICONINFORMATION);*/
					hWndListBox = hWndListBox1;
					hWndEdit = hWndEdit1;
					selectedFile = selectedFile1;
					path = path1;
