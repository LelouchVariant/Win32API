...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		for (int i = 0; i < 9; i++)
		{
			for (int j = 0; j < 9; j++)
			{
				...
				SendMessage(editBox[i][j], EM_SETLIMITTEXT, 1, 0); //Ограничение 1 символ!!!
	...
	case WM_PAINT:
	{
		...
		SetTextColor(hCmpDC, RGB(255, 127, 0));
	...
	case WM_COMMAND:
	{
		...
		case ID_STATISTIC:
		{
			...
			if ((fopen_s(&Fz, "statistic.dat", "rb")) == NULL)
			{
				...
				D_f = _fileno(Fz); //Возвращает дискриптор
				len = _filelength(D_f); // возвращает выраженную в байтах длину файла
				kol = len / size;
		...
		case ID_ABOUT:
		{
			MessageBox(hWnd, L"Program: Sudoku v. 1.0\nDeveloper: Lelouch Variant\nE-mail: 404.variant@gmail.com", L"About", MB_HELP);
		}
		break;
		default:
